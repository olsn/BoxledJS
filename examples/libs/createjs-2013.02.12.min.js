    ...
    a.dispatchEvent = function(b, g) {
        ...
        if (b && a) {
            ...
            for (var a = a.slice(), c = 0, f = a.length; c < f; c++) {
                var h = a[c];
                h instanceof Function ? d = d || h.apply(null, [b]) : h.handleEvent && (d = d || h.handleEvent(b))
            }
        }
        return !!d
    };
...
(function() {
    ...
    a.appendTransform = function(b, g, d, a, e, f, h, k, m) {
        ...
        f || h ? (f *= c.DEG_TO_RAD, h *= c.DEG_TO_RAD, this.append(Math.cos(h), Math.sin(h), -Math.sin(f), Math.cos(f), b, g), this.append(e * d, p * d, -p * a, e * a, 0, 0)) : this.append(e * d, p * d, -p * a, e * a, b, g);
        ...
        if (null != b) {
            ...
            if (null != (d = b.animations)) {
                ...
                for (var h in d) {
                    ...
                    } else b.frequency = f.frequency, b.next = f.next, g = f.frames, a = b.frames = "number" == typeof g ? [g] : g.slice(0);
...
(function() {
    ...
    b.decodePath = function(b) {
        for (var d = [this.moveTo, this.lineTo, this.quadraticCurveTo, this.bezierCurveTo, this.closePath], c = [2, 2, 4, 6, 0], e = 0, f = b.length, h = [], k = 0, m = 0, p = a.BASE_64; e < f;) {
            var q = b.charAt(e),
                s = p[q],
                t = s >> 3,
                u = d[t];
...
(function() {
    ...
    a.updateCache = function(b) {
        ...
        f.save();
        b || f.clearRect(0, 0, a.width, a.height);
...
(function() {
    ...
    a.draw = function(b, a) {
        ...
        for (var d = this.children.slice(0), c = 0, e = d.length; c < e; c++) {
            var f = d[c];
    ...
    a._getObjectsUnderPoint = function(b, a, d, j) {
        ...
        if (!this.hitArea && (this.cacheCanvas && k) && (this.getConcatenatedMatrix(h), e.setTransform(h.a, h.b, h.c, h.d, h.tx - b, h.ty - a), e.globalAlpha = h.alpha, this.draw(e), this._testHit(e))) return f.width = 0, f.width = 1, this;
        for (var m = this.children.length - 1; 0 <= m; m--) {
            ...
            if (p.visible && !(!q && !p.isVisible() || j && !p.mouseEnabled)) {
                ...
                } else if (!j || k || s) if (p.getConcatenatedMatrix(h), q && (h.appendTransform(q.x, q.y, q.scaleX, q.scaleY, q.rotation, q.skewX, q.skewY, q.regX, q.regY), h.alpha = q.alpha), e.globalAlpha = h.alpha, e.setTransform(h.a, h.b, h.c, h.d, h.tx - b, h.ty - a), (q || p).draw(e), this._testHit(e)) {
                    f.width = 0;
                    f.width = 1;
                    if (k) return this;
                    if (d) d.push(p);
...
(function() {
    ...
    a._updatePointerPosition = function(b, a, d) {
        ...
        (c.inBounds = 0 <= a && 0 <= d && a <= e - 1 && d <= f - 1) ? (c.x = a, c.y = d) : this.mouseMoveOutside && (c.x = 0 > a ? 0 : a > e - 1 ? e - 1 : a, c.y = 0 > d ? 0 : d > f - 1 ? f - 1 : d);
        c.rawX = a;
        c.rawY = d;
    ...
    a._handlePointerUp = function(b, a, d) {
        var c = this._getPointerData(b),
            e;
        if (this.onMouseMove || this.hasEventListener("stagemouseup")) e = new createjs.MouseEvent("stagemouseup", c.x, c.y, this, a, b, b == this._primaryPointerID, c.rawX, c.rawY), this.onMouseUp && this.onMouseUp(e), this.dispatchEvent(e);
        var f = c.event;
        ...
        if ((f = c.target) && (f.onClick || f.hasEventListener("click")) && this._getObjectsUnderPoint(c.x, c.y, null, !0, this._mouseOverIntervalID ? 3 : 1) == f) e = new createjs.MouseEvent("click", c.x, c.y, f, a, b, b == this._primaryPointerID, c.rawX, c.rawY), f.onClick && f.onClick(e), f.dispatchEvent(e);
    ...
    a._handlePointerDown = function(b, a, d, c) {
        ...
        if (this.onMouseDown || this.hasEventListener("stagemousedown")) d = new createjs.MouseEvent("stagemousedown", e.x, e.y, this, a, b, b == this._primaryPointerID, e.rawX, e.rawY), this.onMouseDown && this.onMouseDown(d), this.dispatchEvent(d);
        if (c = this._getObjectsUnderPoint(e.x, e.y, null, this._mouseOverIntervalID ? 3 : 1)) if (e.target = c, c.onPress || c.hasEventListener("mousedown")) if (d = new createjs.MouseEvent("mousedown", e.x, e.y, c, a, b, b == this._primaryPointerID, e.rawX, e.rawY), c.onPress && c.onPress(d), c.dispatchEvent(d), d.onMouseMove || d.onMouseUp || d.hasEventListener("mousemove") || d.hasEventListener("mouseup")) e.event = d
    };
...
(function() {
    ...
    a._normalizeFrame = function() {
        ...
        if (b) if (c = b.frames.length, this.currentAnimationFrame >= c) {
            var e = b.next;
            this._dispatchAnimationEnd(b, a, d, e, c - 1) || (e ? this._goto(e) : (this.paused = !0, this.currentAnimationFrame = b.frames.length - 1, this.currentFrame = b.frames[this.currentAnimationFrame]))
    ...
    a._goto = function(b,d) {
        if (isNaN(b)) {
            var a = this.spriteSheet.getAnimation(b);
            a && (this.currentAnimationFrame = d?(d%(a.frames.length-1)):0, this._animation = a, this.currentAnimation = b, this._normalizeFrame())
...
(function() {
    ...
    a._drawText = function(b) {
        ...
        for (var d = String(this.text).split(/(?:\r\n|\r|\n)/), c = this.lineHeight || this.getMeasuredLineHeight(), e = 0, f = 0, h = d.length; f < h; f++) {
...
(function() {
    ...
    c._flip = function(a, b, g, d) {
        ...
        for (k = 0; k < e; k++) {
            ...
            g && (q.x = p.width - q.x - q.width, s.regX = q.width - m.regX);
...
(function() {
    ...
    a.setPosition = function(b, a) {
        ...
        0 != a && 0 < this._actions.length && (this._useTicks ? this._runActions(d, d) : 1 == a && d < e ? (e != this.duration && this._runActions(e, this.duration), this._runActions(0, d, !0)) : this._runActions(e, d));
        c && this.setPaused(!0);
...
(function() {
    ...
    c.calc = function(a, b, g) {
        ...
        for (var d = d[b + 1], h = 0, f = d.length - 1; e > d[h] && h < f;) e -= d[h], h++;
        e = h / ++f + e / (f * d[h]);
        b = 2 * b + 2;
        f = 1 - e;
...
(function() {
    ...
    c.registerSound = function(b, g, e, f) {
        ...
        null != m && (null != m.numChannels && (k = m.numChannels), a.create(h.src, k), null == e || !isNaN(e) ? e = h.data = k || a.maxPerChannel() : e.channels = h.data.channels = k || a.maxPerChannel(), null != m.tag ? h.tag = m.tag : m.src && (h.src = m.src), null != m.completeHandler && (h.completeHandler = m.completeHandler), h.type = m.type);
    ...
    c.play = function(a, b, g, f, h, k, m) {
        ...
        c.playInstance(a, b, g, f, h, k, m) || a.playFailed();
        return a
    };
...
(function() {
    ...
    a.prototype = {
        ...
        play: function(a, b, c, g, h, k) {
            this.cleanUp();
            createjs.Sound.playInstance(this, a, b, c, g, h, k)
        },
        ...
        handleSoundReady: function() {
            null != window.createjs && (this.playState = createjs.Sound.PLAY_SUCCEEDED, this.paused = !1, this.tag.removeEventListener(createjs.HTMLAudioPlugin.AUDIO_READY, this.readyHandler, !1), this.offset >= this.getDuration() ? this.playFailed() : (0 < this.offset && (this.tag.currentTime = 0.0010 * this.offset), -1 == this.remainingLoops && (this.tag.loop = !0), this.tag.play()))
        },
        pause: function() {
    ...
    a.remove = function(a) {
        ...
        if (b) {
            for (; b.length;) {
                ...
                for (f = this._loadQueueBackup.length - 1; 0 <= f; f--) if (h = this._loadQueueBackup[f].getItem(), h.id == c || h.src == c) {
