...
*
* This class has been written in AS3 by Antoan Angelov.
*
...
/*
 * This class was ported to JS by Olaf Horstmann (www.indiegamr.com).
 */
var Box2D = Box2D || {};
Box2D.Ext = Box2D.Ext || {};
(function (scope) {
  ...
  /**
     ...
     * @param verticesArray The vertices of the non-convex polygon, in clockwise order.
     ...
     * @see b2Fixture
     * */
  b2Separator.Separate = function(body,fixtureDef,verticesArray,scale) {
    scale = scale || 32;
    var i,n=verticesArray.length,j,m;
    var vec=new Array,figsVec;
    var polyShape;

    for (i=0; i<n; i++) {
      vec.push(new Box2D.Common.Math.b2Vec2(verticesArray[i].x/scale,verticesArray[i].y/scale));
    }
    ...
    for (i=0; i<n; i++) {
      ...
      m=vec.length;
      for (j=0; j<m; j++) {
        verticesArray.push(new Box2D.Common.Math.b2Vec2(vec[j].x,vec[j].y));
      }

      polyShape=new Box2D.Collision.Shapes.b2PolygonShape;
      polyShape.SetAsArray(verticesArray);
   ...
   * </ul> 
   * */
  b2Separator.Validate = function(verticesArray) {
